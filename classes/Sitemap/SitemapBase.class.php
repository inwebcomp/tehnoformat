...
abstract class SitemapBase
{
  ...
  {
    return "<{$this->root_tag} xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\">".PHP_EOL;
  }
  
  function getClosingRootTag()
  {
    return "</{$this->root_tag}>".PHP_EOL;
  }
  ...
  function wrap($data, $tag, $tab_size = 2)
  {
	if($tag == 'url') $tab_size = 1;
		
	$tab = str_repeat("\t", $tab_size);
	
	if($tag == 'url')
		return $tab."<$tag>".PHP_EOL."{$data}{$tab}</$tag>".PHP_EOL;
	
	return $tab."<$tag>$data</$tag>".PHP_EOL;
  }
