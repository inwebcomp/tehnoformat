<div class="edit wow {%config_animation_effect_edit_block%} panel panel-default">
	...
	<div class="panel-body">
		...
		<form role="form" rel="{%oc%}" action="{%controllerName%}/save" class="ajax-form-request form-horizontal" enctype="multipart/form-data">
            ...
            {%field text/label/"Надпись"/6%}
            
            {%field editor/text/"Текст"/6%}

			<hr class="separator" />
            
            {%field number/time/"Истекает через"/2%}
            
            <hr class="separator" />

			{%field select/type/"Тип"/2%}
            
            <hr class="separator" />
            
            <div class="opener" rel="text"></div>
            <div class="opener" rel="gift">
            	{%controllerdynamic select_item%}
                <hr class="separator" />
            </div>
