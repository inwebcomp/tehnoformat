...
{%if type == "Bool"%}
    ...
    {%else%}
        {%if fieldname == "expired"%}
            <div id="fast-edit-{%object%}-{%fieldname%}">
                <div class="checkbox show_only_label"><input class="styled focus-input" type="checkbox" value="1"{%if value !== "1"%} checked{%endif%} id="{%object%}_checkbox_expired" /><label for="{%object%}_checkbox_expired"><span class="glyphicon glyphicon-ok{%if value !== "1"%} active{%endif%}" aria-hidden="true"></span></label><input type="hidden" value="{%value%}" /></div>
            </div>
        {%else%}
            {%if reverse == "1"%}
