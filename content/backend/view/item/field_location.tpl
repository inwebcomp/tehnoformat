<div class="form-group{%@ifset "err_"._field_name%} has-error{%endif%}">
    ...
    <div class="col-md-{%_field_width%}">
        <select name="params[{%_field_name%}]" class="form-control location-select">
            <option value="0">--</option>
            {%block locations_block%}
                {%list .items%}
                    <option value="{%.ID%}" {%if .ID == location_ID%}selected{%endif%}>{%.padding%}{%.title%}</option>
                {%end%}
             {%end%}
        </select>
...
<script>
    $(".location-select").bind("change", function(){
        if ($(this).val() !== '1')
            $(".district-select").val("");
    });
</script>