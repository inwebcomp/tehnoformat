<div class="form-group{%@ifset "err_"._field_name%} has-error{%endif%}">
    ...
    <div class="col-md-{%_field_width%}">
        <select name="params[{%_field_name%}]" class="form-control" id="type-selector">
            <option value="normal" {%if .type == "normal"%}selected{%endif%}>{%lang Обычный%}</option>
            <option value="video" {%if .type == "video"%}selected{%endif%}>{%lang Видео%}</option>
            <option value="photo" {%if .type == "photo"%}selected{%endif%}>{%lang Фото%}</option>
        </select>
