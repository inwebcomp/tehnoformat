            ...
            {%list items%}
                {%ifset ._first%}
                    <tr>
                        ...
                        {%ifset .name%}<th>{%lang Код%}</th>{%endif%}
                        {%ifset .encoding%}<th>{%lang Кодировка%}</th>{%endif%}
                        <th>{%lang Позиция%}</th>
                    </tr>
                {%endif%}
            {%end%}
        ...
        <tbody>
            {%list items%}
                <tr class="vertical-align{%if .block == 1%} warning{%endif%} with_position">
                    ...
                    {%ifset .encoding%}<th>{%.encoding%}</th>{%endif%}
                    <th class="pos">{%ifset __fast_save%}<div class="input-group input-group-sm"><input name="params[{%.ID%}][pos]" type="text" value="{%.pos%}" class="filter-int form-control small" />{%else%}{%.pos%}{%endif%}</div></th>
                </tr>
            {%end%}
        </tbody>
    </table>
