<div class="form-group{%@ifset "err_"._field_name%} has-error{%endif%}">
    ...
    <div class="col-md-{%_field_width%}">
        <select name="params[{%_field_name%}]" class="form-control district-select">
            <option value="">--</option>
            {%block districts_block%}
                {%list .items%}
                    <option value="{%.ID%}" {%if .ID == district_ID%}selected{%endif%}>{%.padding%}{%.title%}</option>
                {%end%}
             {%end%}
        </select>
