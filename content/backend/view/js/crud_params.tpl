<script type="text/javascript">
	...
	/* Actions */
	$('#{%oc%} .btn').bind('click', function() {

        if ($(this).hasClass("del"))
        { 
        	if (confirm('{%lang Вы действительно желаете удалить выделенные элементы?%}'))
    			return $('#{%oc%}').Request({ controller: '{%controllerName%}', action: 'fast_params_delete', data: $('#params_form').serialize() });
    	}
    	else if ($(this).hasClass("save"))
    	{
    		if (confirm('{%lang Вы действительно желаете сохранить данные?%}'))
    			return $('#{%oc%}').Request({ controller: '{%controllerName%}', action: 'fast_params_save', data: $('#params_form').serialize() });
    	}
