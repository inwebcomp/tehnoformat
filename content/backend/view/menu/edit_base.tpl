<div class="edit wow {%config_animation_effect_edit_block%} panel panel-default">
	...
	<div class="panel-body">
		...
		<form role="form" rel="{%oc%}" action="{%controllerName%}/save" class="ajax-form-request form-horizontal" enctype="multipart/form-data">
            ...
            {%field checkbox/is_link/"Активен"/2%}
            
            <hr class="separator" />
            
            {%field select/type/"Тип"/4%}
            {%field checkbox/auto_title/"Авто-заголовок"/2%}
            
            <hr class="separator" />
            
            <div class="opener" rel="page">
            	{%field pages/page/"Страница"/4%}
            </div>
            <div class="opener" rel="link">
            	{%field text/href/"Ссылка"/6%}
            </div>
            
            <script type="text/javascript">
				$("#{%oc%} select[field=type]").bind("change", function(){
					$("#{%oc%} .edit .opener:not([rel="+$(this).val()+"])").hide();
					$("#{%oc%} .edit .opener[rel="+$(this).val()+"]").show();
				}).change();
			</script>
