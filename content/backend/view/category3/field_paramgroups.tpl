...
<div class="form-group{%@ifset "err_"._field_name%} has-error{%endif%}">
    ...
    <div class="col-md-{%_field_width%}">
        <select name="params[{%_field_name%}][]" field="{%_field_name%}" class="form-control" size="5" multiple>
            <option value="" {%if paramgroups == "0"%}selected{%endif%}>-- {%lang Выбрать главную группу%}</option>
            {%list .items%}
                <option value="{%.ID%}" {%ifinarray .ID,paramgroups_array%}selected{%endif%}>{%.title%}</option>
            {%end%}
        </select>
